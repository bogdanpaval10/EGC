...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    }
    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(2, 4, 1));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(45.0f), glm::vec3(0, 1, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_1)) { // creste inaltime
        bottom += deltaTime;
        top -= deltaTime;
        if (perspective == 0) {
    ...
    if (window->KeyHold(GLFW_KEY_2)) { // scade inaltime
        bottom -= deltaTime;
        top += deltaTime;
        if (perspective == 0) {
            projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
    ...
    if (window->KeyHold(GLFW_KEY_3)) { // creste latime
        left += deltaTime;
        right -= deltaTime;
        if (perspective == 0) {
            projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
    ...
    if (window->KeyHold(GLFW_KEY_4)) { // scade latime
        left -= deltaTime;
        right += deltaTime;
        if (perspective == 0) {
            projectionMatrix = glm::ortho(left, right, bottom, top, zNear, zFar);
